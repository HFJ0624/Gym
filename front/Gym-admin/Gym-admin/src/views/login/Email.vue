...
import { GetEmailValidateCode,Login } from '@/api/login'
  ...
  setup() {
    ...
    const getRules = () => ({
      ...
      emailCode: [
        ...
        { min: 6, max: 6, message: '邮箱验证码长度为6位', trigger: 'blur' }
      ]
    })
    ...
    const state = reactive({
      ...
      getCode: async () => {
        ...
        try {
          const { code, message } = await GetEmailValidateCode(state.model.email)
          if (code === 200) {
            ...
            const timer = setInterval(() => {
              ...
              if (countdown <= 0) {
                ...
                state.codeBtnText = '获取邮箱验证码'
              }
            }, 1000)
          } else {
            ElMessage.error(message || '获取邮箱验证码失败，请稍后重试')
          }
        } catch (error) {
          ElMessage.error('获取邮箱验证码失败，请稍后重试')
        }
      },
      submit: () => {
        if (state.loading) {
          ...
          if (valid) {
            ...
            try {
              ...
              if (+code === 200) {
                ...
                if (ctx?.$message) {
                  ctx.$message.error(message || '登录失败，请检查邮箱和验证码')
                }
              }
            } catch (err) {
              if (ctx?.$message) {
