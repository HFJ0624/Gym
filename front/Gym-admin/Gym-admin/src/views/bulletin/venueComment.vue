    ...
    <div class="search-div">
        ...
        <el-form label-width="70px" size="small">
            ...
            <el-row style="display:flex">
                <el-button type="primary" size="small" @click="searchVenueComment">
                    搜索
                </el-button>
        ...
        <el-dialog v-model="dialogVisible" title="添加或修改公告评论" width="460px" center>
            <el-form label-width="120px">
                ...
                <!-- 公告标题选择器 -->
                <el-form-item label="场馆名称">
                    <el-select v-model="venueComment.venueId" placeholder="请选择场馆名称">
                        <el-option v-for="item in venueList" :key="item.id" :label="item.venueName" :value="item.id" />
                    </el-select>
                </el-form-item>

                <el-form-item label="内容">
                    <el-input  v-model="venueComment.content" placeholder="请评论内容"/>
                </el-form-item>

                <el-form-item label="评价">
                    <el-select v-model="venueComment.esteem" placeholder="请选择评论评价">
...
import { GetVenueCommentListByPage,SaveVenueComment,UpdateVenueComment,DeleteVenueCommentById } from '@/api/VenueComment';
...
// 搜索按钮点击事件处理函数
const searchVenueComment = () => {
    fetchData();
}

// 远程调用后端分页查询接口
const fetchData = async () => {
    const {data , code , message } = await GetVenueCommentListByPage(pageParams.value.page , pageParams.value.limit , queryDto.value) ;
...
// 修改按钮点击事件处理函数
const editShow = (row) => {
    // 先从 row 中获取 venueId，如果没有则从 venueList 中根据 venueName 匹配
    let targetVenueId = row.venueId
    if (!targetVenueId && row.venueName) {
        const matchedVenue = venueList.value.find(venue => venue.venueName === row.venueName)
        if (matchedVenue) {
            targetVenueId = matchedVenue.id
        }
    }
...
const venueComment = ref(defaultForm)
...
// 提交按钮事件处理函数
const submit = async () => {
    if(!venueComment.value.id) {
        const {code , message , data} = await SaveVenueComment(venueComment.value) 
        if(code === 200) {
            dialogVisible.value = false
            ElMessage.success('新增场馆评论成功')
            fetchData()
        }else {
            ElMessage.error(message)
        }
    }else {
        const {code , message , data} = await UpdateVenueComment(venueComment.value) 
        if(code === 200) {
            dialogVisible.value = false
            ElMessage.success('修改场馆评论成功')
            fetchData()
        }else {
            ElMessage.error(message)
        }   
...
const deleteById = (row) => {
    ElMessageBox.confirm('此操作将永久删除该记录, 是否继续?', 'Warning', {
       ...
       const {code } = await DeleteVenueCommentById(row.id)
       if(code === 200) {
            ElMessage.success('删除场馆评论成功')
            pageParams.value.page = 1
            fetchData()
       }
    })
